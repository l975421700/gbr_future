    ...
    for ids in ds_names:
        ...
        if ids in ['access-am3-configs', 'am3-plus4k', 'am3-climaerosol', 'am3-climaerop4k']:
            ...
            if not istream is None:
                ...
                ds_data['ann'][ids] = ds[ivar].squeeze().rename({'pressure_0': 'pressure', 'lat_v': 'lat'}).resample({'time': '1YE'}).map(time_weighted_mean).compute()
                print(np.nanmin(ds_data['ann'][ids])) #0
                print(np.nanmax(ds_data['ann'][ids])) #0
