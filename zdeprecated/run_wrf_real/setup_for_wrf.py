...
# configFile = '/home/563/qg8515/code/gbr_future/run_wrf_real/config.json'
...
assert config['analysis_source'] in ['ERA5', 'ERAI', 'FNL'], 'Key analysis_source must be one of ERA5, ERAI or FNL'
...
for ind_job in range(number_of_jobs):
    ...
    print("Start preparation for the run beginning {}".format(job_start_usable.strftime('%Y-%m-%d-%H')))
    ##
    ...
    if not config['only_edit_namelists']:
        if not wrfInitFilesExist:
            ...
            for iDom in range(nDom):
                ...
                geoPath = os.path.join(config["run_dir"],geoFile)
            ...
            if geoFilesExist:
                ...
                os.chdir(run_dir_with_date)
                print("\t\tRun geogrid at {}".format(datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')))
                ...
                for iDom in range(nDom):
                    ...
                    dst = os.path.join(config["run_dir"], geoFile)
                    shutil.move(src, dst)
            ##
            ...
            for iDom in range(nDom):
                ...
                src = os.path.join(config["run_dir"], geoFile)
            ...
            if not os.path.exists(config["metem_dir"]):
                ...
                for hour in range(0,run_length_total_hours+1,1):
            ...
            if not metemFilesExist:
                ...
                if config['analysis_source'] == 'ERA5':
                    print('Using ERA5 data')
                    
                    linkGribCmds = ['./link_grib.csh', config['era5_input']]
                elif config['analysis_source'] == 'ERAI':
                    ...
                    if config['use_high_res_sst_data']:
                        ...
                        print("\t\tRun link_grib for the SST data at {}".format(datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')))
                        ...
                        print("\t\tRun ungrib for the SST data at {}".format(datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')))
                ...
                ##
                purge(run_dir_with_date, 'GRIBFILE*')
                print("\t\tRun link_grib at {}".format(datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')))
                p = subprocess.Popen(linkGribCmds, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdout, stderr = p.communicate()
                stdout = decode_bytes(stdout) 
                stderr = decode_bytes(stderr)
                ##
                f = open('link_grib.log.stdout', 'w')
                f.writelines(stdout)
                f.close()
                ##
                f = open('link_grib.log.stderr', 'w')
                f.writelines(stderr)
                f.close()
                ...
                purge(run_dir_with_date, f'{WPSnml['ungrib']['prefix']}:*')
                ## with open('ungrib.log.era', 'w') as output_f:
                print("\t\tRun ungrib at {}".format(datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')))
                p = subprocess.Popen(['./ungrib.exe'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
                stdout, stderr = p.communicate()
                stdout = decode_bytes(stdout) 
                stderr = decode_bytes(stderr)
                ##
                f = open('ungrib.log.stdout', 'w')
                f.writelines(stdout)
                f.close()
                ##
                f = open('ungrib.log.stderr', 'w')
                f.writelines(stderr)
                f.close()
                ...
                # Run metgrid
                #############
                print("\t\tRun metgrid at {}".format(datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')))
    ...
    if (not config['only_edit_namelists']) and (not wrfInitFilesExist):
        ...
        print("\t\tRun real.exe at {}".format(datetime.datetime.now(datetime.UTC).strftime('%Y-%m-%d %H:%M:%S')))
    ...
    substitutions = {'RUN_DIR': run_dir_with_date,
                     'RUNNAME': config['run_name'],
                     'STARTTIME' : job_start_usable.strftime('%Y%m%d%H'),
