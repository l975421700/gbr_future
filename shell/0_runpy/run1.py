...
ds1 = xr.open_dataset(f'scratch/data/sim/um/barpa_c/{var}/{var}_hourly_{year}{month:02d}.nc', chunks={})
ds2 = xr.open_dataset(f'/g/data/ob53/BARPA2/output/reanalysis/AUST-04/BOM/ERA5/historical/hres/BARPA-C/v1/1hr/{var}/latest/{var}_AUST-04_ERA5_historical_hres_BOM_BARPA-C_v1_1hr_{year}{month:02d}-{year}{month:02d}.nc', chunks={})[var]
if var in ['pr', 'evspsbl', 'evspsblpot']:
...
for var in ['cll']:
    ...
    fl = sorted(glob.glob(f'/g/data/ob53/BARPA2/output/reanalysis/AUST-04/BOM/ERA5/historical/hres/BARPA-C/v1/1hr/{var}/latest/*'))[:540]
...
for var in ['cll']:
    ...
    for year in np.arange(1979, 2024, 1):
        ...
        for month in np.arange(1, 13, 1):
            print(f'#-------- {month}')
            # year=2020; month=1
            ifile = sorted(glob.glob(f'/g/data/ob53/BARPA2/output/reanalysis/AUST-04/BOM/ERA5/historical/hres/BARPA-C/v1/1hr/{var}/latest/{var}_AUST-04_ERA5_historical_hres_BOM_BARPA-C_v1_1hr_{year}{month:02d}-{year}{month:02d}.nc'))[0]
            ofile = f'{odir}/{var}_hourly_{year}{month:02d}.nc'
