for year in $(seq 2021 2021); do
    for month in $(seq 4 4); do
        # month=$(printf '%02d' $month)
        echo $year $month
        qsub -v year=$year,month=$month <<EOF
#PBS -N qjob3_${year}$(printf '%02d' $month)
#PBS -q normal
...
# /g/data/xp65/public/apps/med_conda_scripts/analysis3-25.10.d/bin/python code/gbr_future/shell/0_runpy/run5.py -y $year -m $month
