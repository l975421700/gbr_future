...
for var2 in ['hus', 'ta', 'ua', 'va', 'hur', 'theta', 'theta_e']:
    ...
    if var2 in ['hus', 'ta', 'ua', 'va']:
        print('get ERA5')
        ds['ERA5'][var2] = xr.open_dataset(f'/g/data/rt52/era5/pressure-levels/reanalysis/{var1}/{year}/{var1}_era5_oper_pl_{year}{month:02d}01-{year}{month:02d}{calendar.monthrange(year, month)[1]}.nc')[var1].sel(time=date, level=slice(ptop, 1000)).sel(longitude=slon, latitude=slat, method='nearest').rename({'level': 'pressure'}).compute()
        if var2 in ['hus']:
            ds['ERA5'][var2] *= 1000
        elif var2 in ['ta']:
            ds['ERA5'][var2] -= zerok
        ...
        if 'UM' in dss:
            try:
                ds['UM'][var2] = hk_um_z10_3H[var2].sel(time=date, cell=hp.ang2pix(2**10, slon, slat, nest=True, lonlat=True), method='nearest').sel(pressure=slice(ptop, 1000)).compute()
                if var2 in ['hus']:
                    ds['UM'][var2] *= 1000
                elif var2 in ['ta']:
                    ds['UM'][var2] -= zerok
...
for var2 in ['hus', 'ta', 'ua', 'va', 'hur', 'theta', 'theta_e']:
    ...
    if var2 in ['hus', 'ta', 'ua', 'va']:
        print('get ERA5')
        ds['ERA5'][var2] = xr.open_dataset(f'/g/data/rt52/era5/pressure-levels/reanalysis/{var1}/{year}/{var1}_era5_oper_pl_{year}{month:02d}01-{year}{month:02d}{calendar.monthrange(year, month)[1]}.nc')[var1].sel(level=slice(ptop, 1000)).sel(longitude=slon, latitude=slat, method='nearest').rename({'level': 'pressure'})
        if var2 in ['hus']:
            ds['ERA5'][var2] *= 1000
        elif var2 in ['ta']:
            ds['ERA5'][var2] -= zerok
        ...
        print('get BARRA-R2')
        ds['BARRA-R2'][var2] = xr.open_mfdataset(sorted(glob.glob(f'/g/data/ob53/BARRA2/output/reanalysis/AUS-11/BOM/ERA5/historical/hres/BARRA-R2/v1/1hr/{var2}[0-9]*[!m]/latest/{var2}[0-9]*{year}{month:02d}-{year}{month:02d}.nc')), parallel=True, preprocess=lambda ds: std_func(ds, var=var2))[var2].sel(pressure=slice(ptop, 1000)).sel(lon=slon, lat=slat, method='nearest')
        ...
        print('get UM')
        if 'UM' in dss:
            ds['UM'][var2] = hk_um_z10_3H[var2].sel(cell=hp.ang2pix(2**10, slon, slat, nest=True, lonlat=True), method='nearest').sel(pressure=slice(ptop, 1000), time=slice(f'{year}-{month}', f'{year}-{month}'))
            if var2 in ['hus']:
                ds['UM'][var2] *= 1000
            elif var2 in ['ta']:
                ds['UM'][var2] -= zerok
