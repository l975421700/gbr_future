...
ds['d11km'] = preprocess_umoutput(ds['d11km'])
ds['d4p4km'] = preprocess_umoutput(ds['d4p4km'])
...
for istash in [istash for istash in ds['d4p4km'].rename(var_stash).data_vars if istash.startswith('STASH')]:
    print(f'#-------------------------------- {istash}')
    print(ds['d4p4km'][istash])

for istash in [istash for istash in ds['d11km'].rename(var_stash2).data_vars if istash.startswith('STASH')]:
    print(f'#-------------------------------- {istash}')
...
# TO-DO
remove: 1, TOT PRECIP RATE AFTER TSTEP  KG/M2/S,
        2, CLOUD DROP NUMBER CONC. /m3
        3, TOTAL RAINFALL RATE: LS+CONV KG/M2/S
        4, TOTAL SNOWFALL RATE: LS+CONV KG/M2/S
        5, TOTAL PRECIPITATION RATE     KG/M2/S
        6, DOWNWARD LW RAD FLUX: SURFACE
...
Warning: no variable
#-------- d4p4km
CLOUD ICE (CRYSTALS) AFTER TIMESTEP
